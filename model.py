...
def self_attention(x,k=6,name='self_attention'):
...
def encoder1(inputs,dim = 8):
    ...
    with tf.variable_scope('encoder1', reuse=reuse):
        ...
        output = SA_ResidualBlock('encoder1.SA_Res1', dim, 2*dim, 3, output, sa=True ,  resample='down')
        output = SA_ResidualBlock('encoder1.SA_Res2', 2*dim, 4*dim, 3, output, sa=True,   resample='down')
        output = SA_ResidualBlock('encoder1.SA_Res3', 4*dim, 8*dim, 3, output,  sa=True,  resample='down')
...
def encoder2(inputs,  dim=8, ):
    ...
    with tf.variable_scope('encoder2', reuse=reuse):
        ...
        output = SA_ResidualBlock('encoder2.SA_Res1', dim, 2*dim, 3, output,  sa=True ,  resample='down')
        output = SA_ResidualBlock('encoder2.SA_Res2', 2*dim, 4*dim, 3, output, sa=True ,   resample='down')
        output = SA_ResidualBlock('encoder2.SA_Res3', 4*dim, 8*dim, 3, output,  sa=True , resample='down')
...
def decoder(inputs,dim=8):
    ...
    with tf.variable_scope('decoder', reuse=reuse):
        ...
        output = SA_ResidualBlock('decoder.SA_Res2', 8*dim, 4*dim, 3, output,  sa=True ,   resample='up')
        output = SA_ResidualBlock('decoder.SA_Res3', 4*dim, 2*dim, 3, output,  sa=True ,  resample='up')
        output = SA_ResidualBlock('decoder.SA_Res4', 2*dim, 1*dim, 3, output,  sa=True ,  resample=None)
        output = tf.nn.relu(output)
...
def Discriminator(inputs, dim=64 ):
    ...
    with tf.variable_scope('Discriminator', reuse=reuse):
        ...
        output = SA_ResidualBlock('Discriminator.SA_Res2', 2*dim, 4*dim, 3, output,  sa=True ,resample='down')
        output = SA_ResidualBlock('Discriminator.SA_Res3', 4*dim, 8*dim, 3, output, sa=True , resample='down')
        output = SA_ResidualBlock('Discriminator.SA_Res4', 8*dim, 8*dim, 3, output,  sa=True ,resample=None)
        disc_img = SA_ResidualBlock('Discriminator.SA_Res5', 8*dim, 8*dim, 3, output, sa=True, resample=None)
